import { DexFees, Fee, TransferFee } from '../types/bnb/bnb-client.types'
...
export const isFee = (v: Fee | TransferFee | DexFees): v is Fee =>
  Boolean((v as Fee)?.msg_type) && (v as Fee)?.fee !== undefined && (v as Fee)?.fee_for !== undefined
...
export const isTransferFee = (v: Fee | TransferFee | DexFees): v is TransferFee =>
  isFee((v as TransferFee)?.fixed_fee_params) && Boolean((v as TransferFee)?.multi_transfer_fee)
